...
#Join audio channels to only one - PURE PYTHON VERSION (no ffmpeg needed)
def joinAudioChannels(path):
    """Convert stereo audio to mono without requiring ffmpeg"""
    print("  Converting stereo to mono (pure Python method)...")
    ...
    if len(data.shape) == 1:
        ...
        return (path, samplerate, data)
    
    # Convert stereo to mono by averaging channels
    if len(data.shape) == 2:
        # Average all channels
        mono_data = np.mean(data, axis=1, dtype=data.dtype)
        print(f"  Converted from {data.shape[1]} channels to mono")
    else:
        sys.exit("Unsupported audio format!")
    
    # Save the mono version
    outPath = makeFileName("mono", path)
    wavfile.write(outPath, samplerate, mono_data)
    print(f"  Saved mono audio to: {outPath}")
    
    return (outPath, samplerate, mono_data)

#Join audio channels using ffmpeg (fallback if available)
def joinAudioChannelsFFmpeg(path):
    """Original ffmpeg version - kept as fallback"""
    outPath = makeFileName("mono", path)
    
    try:
        if platform.system() == "Linux":
            cmdffmpeg_L = "ffmpeg -y -i {} -ac 1 -f wav {}".format(path, outPath)
            os.system(cmdffmpeg_L)
        elif platform.system() == "Windows":
            cmdffmpeg_W = "./ffmpeg/bin/ffmpeg.exe -y -i {} -ac 1 -f wav {}".format(path, outPath)
            sp.call(cmdffmpeg_W)
        
        tupleMono = readWavFile(outPath)
        return tupleMono
    except Exception as e:
        print(f"  FFmpeg not available, using Python method: {str(e)}")
        return joinAudioChannels(path)
...
def FFT(tupleAudio):
    ...
    fftabs = abs(datafft)
    ttl = list(tupleAudio[AUDIO_DATA].shape)
    shape = ttl.pop()
...
if __name__ == "__main__":
    ...
    print("Is the audio mono? ", isMono(tupleAudio[AUDIO_DATA]))
